...
#Falta saber como verificar o que tem no local{-Verifica Base-}
tipoBase :: String -> String -> Char
tipoBase x y | baseCT = baseCT 
             | baseIAPA = baseIAPA 
             | baseBMT = baseBMT 
             | baseBPC = baseBPC 

#Falta saber como verificar o que tem no local{-Verifica Acerto-}
acertou :: String -> String -> Bool
acertou x y | (tiverAlgo == true) = True
            | otherwise = False

{-Retorna oque sera marcado no local do tiro-}
marcar :: String -> String -> Char
marcar x y | (x == "z" && y == "z") = wShot
           | otherwise = tipoBase(x y)

#Falta localizar local para alterar a exibicao{-Marca o Tabuleiro-}
tabuleiroMarcar :: String -> String -> Char -> IO()
tabuleiroMarcar x y z | tabuleiro[x][y] = z 

{-Verifica condicao de parada (Retorna False caso as bombas acabem ou destrua todos os inimigos-}
verifica :: Int -> Int -> Bool
verifica x y | (x == 0 && y < 18) = False
             | (x == 0 && y == 18) = False
             | (x > 0 && y == 18) = False
             | (x > 0 && y < 18) = True
...
tiro = do
    ...
    y <- getLine
    if(acertou(x y))
        then tabuleiroMarcar(marcar(x y)) playGame((bombas - 1) (pontos + 1))
        else tabuleiroMarcar(marcar("z" "z")) playGame((bombas - 1) pontos)
    
{-Finaliza Game-}
finaliza :: Int -> String
finaliza x | (x == 18) = showVitoria
           | otherwise = showDerrota

#FaltaDesenha{-Inicia Game-}
playGame :: Int -> Int -> String
playGame bombas pontos = do
         if(verifica(bombas pontos))
                 then tiro(bombas pontos)
                 else finaliza(pontos)
...
main = do
    ...
    if(a == "1")
        then putStrLn "Inicia o jogo" playGame(45 0)
        else showDerrota
